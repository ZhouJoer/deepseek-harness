...
export function Workbench(props: WorkbenchProps) {
  ...
  const command = async (action: SecurityCommand['action']) => {
    ...
    if (activeSession.current === sessionId) setView(next)
  }
                    ...
                    <article key={item.value.id} className={css.card}>
                      ...
                      {item.kind === 'evidence' ? (
                        ...
                        <>
                          ...
                          <p>{item.value.conditions}</p>
                          <button
                            disabled={busy > 0 || item.value.published}
                            onClick={() => void perform(() => command({ kind: 'publish', knowledgeId: item.value.id }))}
                          >
                            {t(item.value.published ? 'published' : 'publish')}
                          </button>
                        </>
                      )}
                    </article>

...
| Web 分析员 | 分析指定 HTTP 证据与入口 | 读取证据；请求计划由协调者在批准后执行 |
...
Web 目标记录操作者选择的靶场实例、origin 和路径前缀。HTTP provider 在批准前固定方法、路径、工具镜像和实例。执行时核对 Docker 资源所有权、镜像标识、仅含两容器的内部隔离网络和目标地址。请求连接实测地址且不跟随重定向。文件 provider 拒绝 Web 资产；原有文件记录继续可读，无需提升日志版本。

web-analyst 角色读取分配的证据并提出入口分析。独立 reviewer 子 Session 将支持/反对证据和不确定性绑定发现内容哈希。确定结论须具有已完成验证计划的同目标完整证据；修改会重置发现并拒绝旧复核。报告快照对应一个项目修订，并通过操作者级 Remote 在未选择 Session 时保持可访问。

靶场在创建资源前保存所有权。重启将未完成实例标记为中断；清理逐一核对资源标签后才移除。重置移除已拥有容器和网络，下次启动改变实例标识。构建和生命周期命令仅允许操作者发起。路线图区分已实现控制、真实容器/模型验收和待实现扫描 provider。

## 源码分析与隔离模拟

源码目录是一个不可变资产。清单保存成员路径和哈希，使跨文件观察共享同一资产身份。源码工具只读取制品字节，返回有界、带行号的证据。文件、Web 与源码 provider 拒绝不兼容的资产类型。

离线 Python 和浏览器验证复用计划审批、子进程监督、执行记录及清理。每份计划固定脚本、源码清单、镜像 ID 和资源限制。一次性容器不能访问网络和设备；浏览器运行器通过请求拦截提供快照文件。报告将这些观察标记为模拟，并保留失败，不将其视为硬件结果。

源码读取可并行。外部 provider 按实际共享实例租约，该环境存在活动运行时，环境生命周期操作拒绝受理。模型项目分页和变更确认独立于持久项目限制大小。子任务摘要在检查证据范围后保存在现有 Session 绑定中。

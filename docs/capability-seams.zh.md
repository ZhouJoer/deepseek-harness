...
| `ctx.securityWorkbench` | `core` | [`experimental-security-analysis`](../packages/experimental/security-analysis) | - | [`experimental-client-ui-security-analysis`](../packages/experimental/client-ui-security-analysis), [`experimental-security-profile`](../packages/experimental/security-profile) | - | 持有项目范围、明确的 Session 角色、持久化检查及不可变验证批准；专用分析与环境 provider 通过领域接口注册。 |
